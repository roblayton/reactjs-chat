...
// docker-machine env machinename | source
...
gulp.task('init-db', ['compose'], shell.task([
  'mongo --host $(docker-machine ip machinename) main db_init.js --quiet'
]));

gulp.task('status', shell.task([
  'docker-compose ps machinename'
]));

gulp.task('stop', shell.task([
  'docker-compose stop machinename'
]));
